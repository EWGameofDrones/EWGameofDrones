...
void joystickDirection(){
  ...
  if(yValue > 600){ //left -40
    direction = 20;
  }
  else if(yValue < 50 && xValue > 0){ //right -30
    direction = 10;
  }
  else if(xValue < 50 && yValue > 0){ //up -10
    direction = 30;
  }
  else if(xValue > 600){ //down -20
    direction = 40;
  }
  else{
    direction = 0;
...
void moveStepper2(){
  ...
  if (direction == 30) { // Joystick moved UP out of the "null zone"
    digitalWrite(motor2DirPin, HIGH); // Set direction to move clockwise
  } 
  else if (direction == 40) { // Joystick moved DOWN out of the "null zone"
