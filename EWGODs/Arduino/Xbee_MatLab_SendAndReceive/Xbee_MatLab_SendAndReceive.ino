...
int fireVerifyLight = A3; //digital pin 17 
...
String jammer = "J"; //for sending jam packets
String handshake = "0";
...
void processRxPacket(ZBRxResponse& rx, uintptr_t)
{
  ...
  else if (type == 2 && b.len() == 2) // null packet sent
  {
    int sensID = b.remove<int>();
    sensorID = String(sensID); 
  ...
  //Jam Packet
  else if (type == 3 && b.len() == 2) // Jam packet sent
  {
    int sensID = b.remove<int>();
    sensorID = String(sensID); 

    packetToSend = sensorID + comma + jammer;
